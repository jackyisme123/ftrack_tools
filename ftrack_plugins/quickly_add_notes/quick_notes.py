...
class quick_notes_gui:
    ...
    def __init__(self, app, auto_record=True):
        '''set GUI'''
        self.login_font = Font(family="New Times Roman", size=11)
        self.wait_font = Font(family='Fixdsys',size=13)
        ...
        app.iconbitmap("./write.ico")
        self.canvas = Canvas(app)
        self.frame = Frame(self.canvas)
        vsb = Scrollbar(app, orient="vertical", command=self.canvas.yview)
        ...
        # mark bind cursor_busy
        fileMenu.add_separator()
        fileMenu.add_command(label='Quit', command=lambda app=app: self.quit(app))
        self.frame1 = Frame(self.frame)
        self.frame1.pack(side=TOP, fill=X, pady=20)
        self.choose_user_entry_text = StringVar()
        self.choose_key_entry_text = StringVar()
        self.choose_url_entry_text = StringVar()
        self.choose_user_label = Label(self.frame1, text='User name:')
        self.choose_user_entry = Entry(self.frame1, textvariable=self.choose_user_entry_text)
        self.choose_key_label = Label(self.frame1, text='API Key:')
        self.choose_key_entry = Entry(self.frame1, textvariable=self.choose_key_entry_text, show="*")
        self.choose_url_label = Label(self.frame1, text='Server URL:')
        self.choose_url_entry = Entry(self.frame1, textvariable=self.choose_url_entry_text)
        ...
        if auto_record:
            try:
                with open(APPDATA+'\\Local\\save.p', 'rb') as f:
                    ...
                    self.api_key = save_info['api_key']
                    self.choose_user_entry_text.set(user_name)
                    self.choose_key_entry_text.set(self.api_key)
                    self.choose_url_entry_text.set(server_url)
            except Exception:
                pass
        self.login_button = Button(self.frame1, text='Login', command=self.login)
        self.login_button.bind("<Button-1>", self.cursor_busy)
        # self.login_button.pack_configure(after=self.choose_key_entry)
        self.login_err_msg = Label(self.frame1, text='Sorry: invalid login info', fg='red')
        self.load_button = Button(self.frame1, text='Load previous session', bg='yellow', command=self.load_project)
        self.load_button.bind("<Button-1>", self.cursor_busy)
        self.login_button.pack(side=LEFT, padx=10)
        # self.load_button.pack_configure(after=self.login_button)
        ...
        self.search_task_button.bind('<Button-1>', self.cursor_busy)
        ...
        self.search_project_button.bind("<Button-1>", self.cursor_busy)
    ...
    def _on_mousewheel(self, event):
        '''mouse wheel event'''
        if self.frame.winfo_height() > 800:
    ...
    def search_project(self):
        ...
        for project in self.projects:
            if project_name.lower() in project['full_name'].lower():
                ...
                project_button.bind("<ButtonRelease-1>", self.choose_project)
                project_button.bind("<ButtonPress-1>", self.cursor_busy)
    ...
    def use_task(self, event):
        ...
        if task_name in self.save_message.keys():
            tkMessageBox.showwarning("Warning", "Task has already existed")
        else:
    ...
    def save_project(self):
        ...
        if(self.save()):
            ...
            tkMessageBox.showwarning("Warning", "Nothing for saving!")

    def load_project(self):
        '''load project'''
        self.cursor_busy(None)
        self.k.stop()
        self.k = ThreadJob(self.save, threading.Event(), AUTO_SAVE_TIME)
        try:
            with open(APPDATA+'\\Local\\save.p', 'rb') as f:
            ...
            tkMessageBox.showwarning("Failure", "Loading failure!")
            self.cursor_normal()
            return
        self.project_id = save_info['project_id']
        self.user_name = save_info['user_name']
        notes_msg = save_info['notes']
        self.server_url = save_info['server_url']
        self.login_err_msg.pack_forget()
        self.api_key = save_info['api_key']
        self.session = get_session(self.server_url, self.api_key, self.user_name)
        self.project = self.session.query("Project where id is {0}".format(self.project_id)).one()
        self.user = self.session.query("User where username is '{0}'".format(self.user_name)).one()
        self.tasks = get_tasks_from_project(self.project)
        self.projects = self.session.query("Project").all()
    ...
    def cursor_busy(self, event):
        self.canvas.config(cursor="wait")
        self.app.title(">>>   Program loading! Please wait for a while!   <<< ")
        self.app.iconbitmap("./loading.ico")

    def cursor_normal(self):
        self.canvas.config(cursor="")
        self.app.title("Quick notes")
        self.app.iconbitmap("./write.ico")
    ...
    #     win.title('Loading...')
    #     Label(win, text='Program is Loading now', font=self.wait_font, fg='red').pack(fill=X, pady=20)
    #     Label(win, text='Please wait for a while!', font=self.wait_font, fg='red').pack(fill=X)
    #     return win
...
def submit_info_to_ftrack(info, session, user, project):
    ...
    if flag:
        try:
            ...
            tkMessageBox.showwarning("Warning", "Sorry, your notes are not submitted!")
            session.rollback()
            raise
    else:
        tkMessageBox.showwarning("Warning", "No submission content!")
